...
int main() {
    ...
    if (x3dh_compute_session_key(
            ...
            (!bob.oneTimePreKeys.empty() ? bob.oneTimePreKeys[0].publicKey : NULL), // Optional one-time pre key.
            alice_session_key) != 0) {
    ...
    if (sodium_memcmp(alice_session_key, bob_session_key, X3DH_SESSION_KEY_BYTES) != 0) {
        std::cerr << "X3DH key agreement FAILED. Session keys do not match." << std::endl;
        return 1;
    }
    ...
    // In a real protocol, both parties exchange their double ratchet DH public keys.
    // For this simulation, each party generates its DH key pair during initialization.
    // Alice uses Bob's double ratchet public key and vice versa.
    ...
    if (!alice.dratchet.encryptMessage(reinterpret_cast<const unsigned char*>(message),
                                         strlen(message),
    ...
    if (!bob.dratchet.decryptMessage(ciphertext, ciphertextLen, nonce,
    ...
    std::cout << "\nBob decrypted the message: " 
              << std::string(reinterpret_cast<char*>(decrypted), decryptedLen) << std::endl;
